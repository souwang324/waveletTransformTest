...
function p = waveletLevelDD137(p, shift)
      ...
      for pixel=1:2 :width
        ...
        if (pixel+3)<width
         ...
         p(line, pixel) =p(line, pixel) +...
             bitshift(-p(line, tap0)+9*p(line,tap1)+9*p(line,tap2)-p(line,tap3)+16, -5);
     end
    end
  ...
  for line=1:2 : height
    ...
    for pixel=1:width
      p(line, pixel) =p(line, pixel) +...
        bitshift(-p(tap0, pixel)+9*p(tap1, pixel)+9*p(tap2,pixel)-p(tap3, pixel)+16, -5);
    end
  end
end
